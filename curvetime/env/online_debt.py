...
from curvetime.db.models import Debts
...
HOLDING_FILE = 'data/models/debts_holding.pkl'
...
class StockEnv(TradingEnv):
    ...
    def _get_observation(self):
        df = self.oracle.get_dataframe(self.frame_count, self.window_size, type='online')
        if not df:
            return None
...
def send_mail(msg):
    ...
    receivers = ['barco@curvetime.cn', 'kenny@curvetime.cn']
    message = """Subject: 弯时X号可转债精选 (CurveTime AI Premium X)
    ...
    #mail.ehlo()
    #mail.starttls()
    mail.login('barco@curvetime.cn', '6yJFu2V3H2uAfzyr')
